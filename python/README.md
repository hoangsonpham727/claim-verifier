...
Every model call routes through `src/grounding/client.py` (the single vendor seam). The API field shapes used throughout are documented in [`../shared/NOTES_api.md`](../shared/NOTES_api.md).
...
This is the Python implementation, rooted at `python/`. Shared, language-neutral
assets live outside it: the Word add-in at `../addin`, sample documents and the
API contract at `../shared`. Run the commands below from this `python/` directory.
...
cp ../.env.example ../.env  # .env lives at the repo root (shared by both backends)
```
...
1. Install the trusted local certs once:
   ...
   ```
2. Serve the add-in files over HTTPS (the add-in lives at the repo root):
   ```bash
   python3 ../addin/serve_https.py     # https://localhost:3001
   ```
3. Start the FastAPI service over HTTPS (command above).
4. Sideload the manifest into Word (run from the repo root, where `package.json` lives):
   ```bash
   npm install
